...
import type { CurrentSaveData, ImportResult } from "./saveTypes";
...
function migrateV0ToV1(raw: LegacySaveDataV0): CurrentSaveData {
  ...
  return {
    version: 1, savedAt: new Date().toISOString(), playTimeSeconds: 0,
    player: { ...INITIAL_PLAYER_STATE, level: typeof raw.playerLevel === "number" ? raw.playerLevel : 1, currentHp: hp },
...
export function migrateSaveData(raw: unknown): ImportResult {
  ...
  if (typeof version === "number" && version > 1) return { success: false, reason: "unsupportedVersion" };
  try {
    const migratedFromVersion = version === undefined || version === 0 ? 0 : undefined;
    const candidate = migratedFromVersion === 0 ? migrateV0ToV1(raw as LegacySaveDataV0) : raw;
