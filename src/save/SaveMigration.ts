...
import type { CurrentSaveData, ImportResult, SaveDataV1, SaveDataV2, SaveDataV3, SaveDataV4, SaveDataV5 } from "./saveTypes";
...
function migrateV0ToV6(raw: LegacySaveDataV0): CurrentSaveData {
...
function migrateV1ToV6(raw: SaveDataV1): CurrentSaveData {
  return {
    ...raw,
    version: 6,
    player: {
...
function migrateV2ToV6(raw: SaveDataV2): CurrentSaveData {
  return { ...raw, version: 6, player: { ...raw.player, name: "" }, inventory: initialInventory(), questProgress: emptyQuestProgress() };
}

function migrateV3ToV6(raw: SaveDataV3): CurrentSaveData {
...
function migrateV4ToV6(raw: SaveDataV4): CurrentSaveData {
...
function migrateV5ToV6(raw: SaveDataV5): CurrentSaveData {
  const inventory: InventoryState = {
    items: { ...raw.inventory.items },
    equippedItemIds: {
      weaponSkin: raw.inventory.equippedItemIds.weaponSkin ?? null,
...
export function migrateSaveData(raw: unknown): ImportResult {
  ...
  if (typeof version === "number" && version > 6) return { success: false, reason: "unsupportedVersion" };
  try {
    const migratedFromVersion =
      version === undefined || version === 0 ? 0 : version === 1 ? 1 : version === 2 ? 2 : version === 3 ? 3 : version === 4 ? 4 : version === 5 ? 5 : undefined;
    const candidate =
