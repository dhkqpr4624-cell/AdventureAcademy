...
import type { CurrentSaveData, ImportResult, SaveDataV1, SaveDataV2, SaveDataV3 } from "./saveTypes";
import { INITIAL_INVENTORY_STATE } from "../game/inventory/inventoryState";
...
function migrateV0ToV4(raw: LegacySaveDataV0): CurrentSaveData {
  ...
  return {
    version: 4, savedAt: new Date().toISOString(), playTimeSeconds: 0,
...
function migrateV1ToV4(raw: SaveDataV1): CurrentSaveData {
  return {
    ...raw,
    version: 4,
    player: {
...
function migrateV2ToV4(raw: SaveDataV2): CurrentSaveData {
  return { ...raw, version: 4, player: { ...raw.player, name: "" }, inventory: initialInventory(), questProgress: emptyQuestProgress() };
}

function migrateV3ToV4(raw: SaveDataV3): CurrentSaveData {
  return { ...raw, version: 4, player: { ...raw.player, name: "" }, questProgress: emptyQuestProgress() };
}
...
export function migrateSaveData(raw: unknown): ImportResult {
  ...
  if (typeof version === "number" && version > 4) return { success: false, reason: "unsupportedVersion" };
  try {
    const migratedFromVersion =
      version === undefined || version === 0 ? 0 : version === 1 ? 1 : version === 2 ? 2 : version === 3 ? 3 : undefined;
    const candidate =
      migratedFromVersion === 0
        ? migrateV0ToV4(raw as LegacySaveDataV0)
        : migratedFromVersion === 1
          ? migrateV1ToV4(raw as SaveDataV1)
          : migratedFromVersion === 2
            ? migrateV2ToV4(raw as SaveDataV2)
            : migratedFromVersion === 3
              ? migrateV3ToV4(raw as SaveDataV3)
            : raw;
