...
type TitleScreenProps = {
  ...
  hasSave: boolean;
  onNewGame: () => void;
  onDebugFloorJump: (floorId: "floor-1" | "floor-2" | "floor-3" | "floor-4" | "floor-5" | "floor-6" | "floor-7" | "floor-8") => void;
};
    ...
    <main className="game-screen title-screen">
      ...
      {debugVisible && <div className="title-debug-floor-jump">
        ...
        {debugOpen && <div className="title-debug-panel" aria-label="던전 층 테스트 이동">
          {([1, 2, 3, 4, 5, 6, 7, 8] as const).map((floor) => <button key={floor} type="button" onClick={() => onDebugFloorJump(`floor-${floor}`)}>Dungeon {floor}</button>)}
        </div>}
      </div>}
