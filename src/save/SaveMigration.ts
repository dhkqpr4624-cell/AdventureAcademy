...
import type { CurrentSaveData, ImportResult, SaveDataV1, SaveDataV2 } from "./saveTypes";
...
function migrateV0ToV3(raw: LegacySaveDataV0): CurrentSaveData {
  ...
  return {
    version: 3, savedAt: new Date().toISOString(), playTimeSeconds: 0,
...
function migrateV1ToV3(raw: SaveDataV1): CurrentSaveData {
  return {
    ...raw,
    version: 3,
    player: {
...
function migrateV2ToV3(raw: SaveDataV2): CurrentSaveData {
  return { ...raw, version: 3, inventory: initialInventory() };
}
...
export function migrateSaveData(raw: unknown): ImportResult {
  ...
  if (typeof version === "number" && version > 3) return { success: false, reason: "unsupportedVersion" };
  try {
    const migratedFromVersion =
      version === undefined || version === 0 ? 0 : version === 1 ? 1 : version === 2 ? 2 : undefined;
    const candidate =
      migratedFromVersion === 0
        ? migrateV0ToV3(raw as LegacySaveDataV0)
        : migratedFromVersion === 1
          ? migrateV1ToV3(raw as SaveDataV1)
          : migratedFromVersion === 2
            ? migrateV2ToV3(raw as SaveDataV2)
            : raw;
